...
import edu.only.ksp.domain.aggregates.role.QRole
import edu.only.ksp.domain.aggregates.role.Role
import edu.only.ksp.domain.aggregates.role.Role_
...
class SRole(
    ...
    fun rolePermissions(): Schema.Field<List<edu.only.ksp.domain.aggregates.role.RolePermission>> {
    ...
    fun joinRolePermission(joinType: Schema.JoinType): SRolePermission {
        ...
        @Suppress("UNCHECKED_CAST")
        val join =
            (root as Root<Role>).join<Role, edu.only.ksp.domain.aggregates.role.RolePermission>("rolePermissions", type)
